...
_topLevelDataBase = {
        'file_fullPath': np.empty(0,dtype=np.unicode),
        'file_path': np.empty(0,dtype=np.unicode),
        'file_name': np.empty(0,dtype=np.unicode),
        'file_ext': np.empty(0,dtype=np.unicode),
        'file_name_without_ext': np.empty(0,dtype=np.unicode),
...
class PsortDataBase():
    ...
    def init_slotsDataBase(self, total_slot_num=None, index_slot_edges=None):
        ...
        for counter_slot in range(total_slot_num):
            ...
            self._grandDataBase[counter_slot]['index_start_on_ch_data'][0] = index_slot_edges[counter_slot]
            self._grandDataBase[counter_slot]['index_end_on_ch_data'][0] = index_slot_edges[counter_slot+1]
        self._grandDataBase.append(deepcopy(_singleSlotDataBase))
    ...
    def load_mat_dataBase(self, file_fullPath):
        ...
        index_slot_edges = np.round(np.linspace(0, data_size, total_slot_num+1, endpoint=True)).astype(int)
        ...
        self._topLevelDataBase['sampleRate'][0] = int(data_mat['ch_info']['header']['sampleRate'])
    ...
    def load_continuous_dataBase(self, file_fullPath):
        ...
        ch_time_first_element = float(data_continuous['timestamps'][0])/float(data_continuous['header']['sampleRate'])
        ...
        self._topLevelDataBase['index_slot_edges'] = deepcopy(index_slot_edges)
        self._topLevelDataBase['sampleRate'][0] = int(data_continuous['header']['sampleRate'])
    ...
    def get_current_slot_data_time(self):
        index_start_on_ch_data = self._currentSlotDataBase['index_start_on_ch_data'][0]
        index_end_on_ch_data = self._currentSlotDataBase['index_end_on_ch_data'][0]
        return\
            self._topLevelDataBase['ch_data'][index_start_on_ch_data:index_end_on_ch_data],\
            self._topLevelDataBase['ch_time'][index_start_on_ch_data:index_end_on_ch_data]

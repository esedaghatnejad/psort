...
def _get_compression_filters(compression="default"):
    ...
    if (
        ...
        try:
            ...
            warnings.warn(
                (
                    "(deepdish.io.save) Missing compression method {}: "
...
def _save_pickled(handler, group, level, name=None):
    warnings.warn(
        (
            "(deepdish.io.save) Pickling {}: This may cause "
...
def load(path, group=None, sel=None, unpack=False):
    ...
    with tables.open_file(path, mode="r") as h5file:
        ...
        if group is not None:
            ...
            grp = h5file.root
            auto_unpack = (
                DEEPDISH_IO_UNPACK in grp._v_attrs and grp._v_attrs[DEEPDISH_IO_UNPACK]
            )

...
class SlotBoundaryWidget(QtWidgets.QWidget):
    def __init__(self, parent=None):
        ...
        self._workingDataBase = {
            ...
            "index_slot_edges": np.linspace(
                0, int(50e2), num=10 + 1, endpoint=True, dtype=np.uint32
            ),
            "ch_data": np.random.normal(loc=0.0, scale=1.0, size=int(50e2)),
            "sample_rate": np.full((1), int(30e3), dtype=np.uint32),
            "restart_mode": np.array(["hard"], dtype=np.unicode_),
        }
        self.infLine_list = []
    ...
    def build_slotBoundary_Widget(self):
        ...
        self.layout_slotBoundary_toolbar.addWidget(
            self.label_slotBoundary_resetModeDescription
        )
        self.layout_slotBoundary_toolbar.addWidget(
            self.comboBx_slotBoundary_restartMode
        )
    ...
    def connect_slotBoundary_signals(self):
        self.pushBtn_slotBoundary_addSlot.clicked.connect(self.onAddSlot_Clicked)
        self.spinBx_slotBoundary_numSlots.valueChanged.connect(
            self.onSpinBxNumSlots_ValueChanged
        )
        self.comboBx_slotBoundary_restartMode.currentIndexChanged.connect(
            self.onRestartMode_IndexChanged
        )
        return 0

    def init_slotBoundary_plot(self):
        self.plot_slotBoundary_mainPlot.setTitle("Y: Raw data(uv) | X: Index")
        self.pltData_slotBoundary_chData = self.plot_slotBoundary_mainPlot.plot(
            np.zeros((0)),
            np.zeros((0)),
            name="ch_data",
            pen=pg.mkPen(color="k", width=1, style=QtCore.Qt.SolidLine),
        )
        self.infLine_crosshair_vLine = pg.InfiniteLine(
            pos=0.0, angle=90, pen=(255, 0, 255, 255), movable=False, hoverPen="g"
        )
        self.infLine_crosshair_hLine = pg.InfiniteLine(
            pos=0.0, angle=0, pen=(255, 0, 255, 255), movable=False, hoverPen="g"
        )
        self.infLine_start = pg.InfiniteLine(
            pos=+0.0,
            angle=90,
        ...
        self.infLine_end = pg.InfiniteLine(
            pos=+0.0,
            angle=90,
            pen=pg.mkPen(color=(0, 0, 255, 255), width=2, style=QtCore.Qt.SolidLine),
            movable=False,
        ...
        self.plot_slotBoundary_mainPlot.addItem(
            self.infLine_crosshair_hLine, ignoreBounds=True
        )
        self.plot_slotBoundary_mainPlot.addItem(
            self.infLine_crosshair_vLine, ignoreBounds=True
        )
    ...
    def onSpinBxNumSlots_ValueChanged(self):
        ...
        self._workingDataBase["index_slot_edges"] = np.linspace(
            0, len_data, num=total_slot_num + 1, endpoint=True, dtype=np.uint32
        )
    ...
    def onRestartMode_IndexChanged(self):
        if self.comboBx_slotBoundary_restartMode.currentIndex() == 0:
            self._workingDataBase["restart_mode"] = np.array(
                ["hard"], dtype=np.unicode_
            )
        elif self.comboBx_slotBoundary_restartMode.currentIndex() == 1:
            self._workingDataBase["restart_mode"] = np.array(
                ["soft"], dtype=np.unicode_
            )
        return 0
    ...
    def onInfLine_positionChangeFinished(self):
        ...
        if flag_rebuild_infLine_list:
            self._workingDataBase["index_slot_edges"] = np.concatenate(
                ([0], infLine_values, [len_data])
            )
            self.clear_infLine_list()
            self.build_infLine_list()
        self._workingDataBase["index_slot_edges"] = np.concatenate(
    ...
    def set_chData(self):
        ...
        self.pltData_slotBoundary_chData.setData(
            idx_values[::step_size], ch_data[::step_size]
        )
    ...
    def build_infLine_list(self):
        ...
        for counter_edges, value in enumerate(index_slot_edges):
            ...
            self.plot_slotBoundary_mainPlot.addItem(
                self.infLine_list[counter_infLine_list], ignoreBounds=True
            )
            self.infLine_list[counter_infLine_list].sigPositionChangeFinished.connect(
                self.onInfLine_positionChangeFinished
            )
            counter_infLine_list += 1
        self.spinBx_slotBoundary_numSlots.valueChanged.disconnect(
            self.onSpinBxNumSlots_ValueChanged
        )
        len_data = len(self._workingDataBase["ch_data"])
        sample_rate = self._workingDataBase["sample_rate"][0]
        ...
        self.label_slotBoundary_duration.setText(
            f"Avg slot duration: {avg_slot_duration}s."
        )
    ...
    def clear_infLine_list(self):
        for counter in range(len(self.infLine_list) - 1, -1, -1):
            self.infLine_list[counter].sigPositionChangeFinished.disconnect(
                self.onInfLine_positionChangeFinished
            )
    ...
    def add_infLine(self, new_value):
        ...
        if (new_value <= 0) or (new_value >= len_data):
            return 0
        infLine_values = self.get_infLine_values()
        self._workingDataBase["index_slot_edges"] = np.sort(
            np.concatenate(([0], infLine_values, [len_data], [new_value]))
        )
        self.clear_infLine_list()
        self.build_infLine_list()
        return 0

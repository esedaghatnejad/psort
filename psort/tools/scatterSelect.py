...
class ScatterSelectWidget(QWidget):
    ...
    def init_scatterSelect_plot(self):
        # scatterSelect Plot
        self.pltData_scatterSelectPlot = self.plot_scatterSelect_mainPlot.plot(
            np.zeros((0)),
            np.zeros((0)),
            name="scatterSelect",
            pen=None,
        ...
        self.pltData_scatterSelectPlotTemplate = self.plot_scatterSelect_mainPlot.plot(
            np.zeros((0)),
            np.zeros((0)),
            name="scatterSelectTemplate",
            pen=None,
            symbol="o",
            symbolSize=3,
            symbolBrush=(0, 0, 0, 255),
        ...
        self.pltData_scatterSelectPlot_ROI2 = self.plot_scatterSelect_mainPlot.plot(
            np.zeros((0)),
            np.zeros((0)),
            name="ROI2",
        ...
        for counter in range(len(self.list_color)):
            self.pltData_scatterSelectPlot_list.append(
                self.plot_scatterSelect_mainPlot.plot(
                    np.zeros((0)),
                    np.zeros((0)),
                    pen=None,
                    symbol="o",
            ...
            self.pltText_scatterSelectPlot_list.append(
                pg.TextItem(
                    str(counter + 1),
            ...
            self.plot_scatterSelect_mainPlot.addItem(
                self.pltText_scatterSelectPlot_list[counter], ignoreBounds=True
            )
        ...
        if evt[0].button() == QtCore.Qt.LeftButton:
            ...
            if self.plot_scatterSelect_mainPlot.sceneBoundingRect().contains(pos):
                ...
                if "_manual" in self._workingDataBase["popUp_mode"][0]:
                    self.pltData_scatterSelectPlot_ROI.setData(
                    ...
                    if self._workingDataBase["popUp_ROI_x"].size > 2:
                        ...
                        self.pltData_scatterSelectPlot_ROI2.setData(
    ...
    def set_chData(self):
        if "ss_pca" in self._workingDataBase["popUp_mode"][0]:
            ...
            self.pltData_scatterSelectPlotTemplate.setData(
                np.zeros((0)),
                np.zeros((0)),
                pen=None,
                symbol=None,
    ...
    def scatterSelect_reset_ROI(self):
        ...
        self.pltData_scatterSelectPlot_ROI2.setData(np.zeros((0)), np.zeros((0)))
        for counter in range(len(self.list_color)):
            self.pltData_scatterSelectPlot_list[counter].setData(
                np.zeros((0)), np.zeros((0))
            )
            self.pltText_scatterSelectPlot_list[counter].hide()
        return 0
    ...
    def scatterSelect_pca_gmm(self):
        ...
        for counter_cluster in range(n_clusters):
            ...
            self.pltText_scatterSelectPlot_list[counter_cluster].setPos(
                centers[counter_cluster, 0], centers[counter_cluster, 1]
            )
            self.pltText_scatterSelectPlot_list[counter_cluster].show()
            self.pltData_scatterSelectPlot_list[counter_cluster].setData(
                pca_mat_2D[index_cluster, 0], pca_mat_2D[index_cluster, 1]
            )
        message = "Which cluster do you want to select?"
        doubleSpinBx_params = {}
        doubleSpinBx_params["value"] = 1.0
        doubleSpinBx_params["dec"] = 0
        doubleSpinBx_params["step"] = 1.0
        doubleSpinBx_params["max"] = n_clusters
        doubleSpinBx_params["min"] = 1.0

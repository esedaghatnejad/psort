...
def load_file_continuous(file_fullPath):
    ...
    ch_time_first_element = float(data_continuous["timestamps"][0]) / float(
        data_continuous["header"]["sampleRate"]
    )
    ...
    if not (ch_time.size == ch_data.size):
        print(
            "Error: <lib.load_file_continuous: "
...
def load_file_matlab(file_fullPath):
    ...
    if "ch_info" in data_mat:
        ...
        sample_rate = int(np.round(1.0 / np.mean(np.diff(ch_time))))
    del data_mat
    return ch_data, ch_time, sample_rate
...
class SaveData(QtCore.QThread):
    ...
    def run(self):
        ...
        if file_ext == ".h5":
            save_file_h5(
                self.file_fullPath, self.ch_data, self.ch_time, self.sample_rate
            )
...
def bandpass_filter(data, sample_rate=None, lo_cutoff_freq=None, hi_cutoff_freq=None):
    ...
    lo_cutoff_wn = float(lo_cutoff_freq) / (float(sample_rate) / 2.0)
    hi_cutoff_wn = float(hi_cutoff_freq) / (float(sample_rate) / 2.0)
...
def find_peaks(data, threshold=None, peakType=None):
    ...
    if (peakType == "max") or (peakType == "Max"):
        ...
        print(
            "Error: <lib.find_peaks: " + "peakType should be either max or min.>",
            file=sys.stderr,
        )
    peak_index_boolean = np.logical_and(
        (np.diff(data) >= 0)[:-1], (np.diff(data) < 0)[1:]
    )
    ...
    peak_index_below_threshold = peak_index_below_threshold[
        data[peak_index_below_threshold] < _threshold
    ]
...
def instant_firing_rate_from_index(index_bool, sample_rate=None):
    ...
    inter_spike_interval = np.diff(index_value) / float(sample_rate)
    np.append(inter_spike_interval, inter_spike_interval[-1])  # ISI in sec
    instant_firing_rate = 1.0 / inter_spike_interval  # IFR in Hz
...
def cross_probability(
    ...
    spike2_bool_size = np.round(
        float(spike1_bool.size) / float(sample_rate) / float(bin_size)
    ).astype(int)
...
def extract_waveform(
    data, spike_bool, sample_rate=None, win_len_before=0.002, win_len_after=0.004
):
    # output is a matrix
    if sample_rate is None:
        sample_rate = 30000.0  # sample_rate in Hz
    ...
    win_len_before_int = np.round(float(win_len_before) * float(sample_rate)).astype(
        int
    )
...
def kmeans(input_data, n_clusters=2, init_val=None):
    ...
    if init_val is None:
        ...
        n_init = 1
    _kmeans = KMeans(n_clusters=n_clusters, init=init_val, n_init=n_init).fit(
        input_data
    )
...
def GaussianMixture(input_data, n_clusters=2, init_val=None, covariance_type="full"):
    ...
    gmm = mixture.GaussianMixture(
        n_components=n_clusters, means_init=init_val, covariance_type=covariance_type
    )
    gmm.fit(input_data)
...
def resample(x_input, y_input, x_output):
    if isinstance(x_output, Number):
        ...
        _num = x_output.size
    y_output, _ = signal.resample(
        y_input, _num, t=x_input, axis=0, window=None, domain="time"
    )
    return y_output
    ...
    for ii in range(n - 1):
        ...
        for i in range(ii + 1, n):
            ...
            if num_label < nknn:
                ...
                continue
            max_num_events = 500
            num_events = min(max_num_events, num_loi, num_label)
            ind_loi = labels == loi
            ...
            distances, indices = (
                NearestNeighbors(n_neighbors=nknn, algorithm="auto")
                .fit(_data)
                .kneighbors()
            )
            ...
            isolation[ii, i] = 200 * np.absolute(0.5 - pct_overlap)
...
def outlier_score(input_data, quant=0.9, knn=20):
    if input_data.size < 1:
        return np.zeros((0)), np.zeros((0))

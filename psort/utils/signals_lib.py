...
def resolve_ss_ss_conflicts(_workingDataBase):
    ...
    for counter_ss in range(_ss_index_int.size):
        ...
        if ss_search_win_int.size > 1:
            ...
            ss_search_win_bool = np.zeros(search_win_inds.shape, dtype=bool)
            ...
            ss_search_win_bool[valid_ind] = not (
                (valid_ind == 0) or (valid_ind == ss_search_win_bool.size)
            )
...
def resolve_cs_slow_cs_slow_conflicts(_workingDataBase):
    ...
    for counter_cs in range(_cs_index_slow_int.size):
        ...
        if _cs_index_slow_local > (_cs_index_slow.size - window_len):
            ...
            continue
        search_win_inds = np.arange(
            _cs_index_slow_local - window_len, _cs_index_slow_local + window_len, 1
        )
        ...
        if cs_search_win_int.size > 1:
            ...
            cs_search_win_bool = np.zeros(search_win_inds.shape, dtype=bool)
            # See the description in resolve_ss_ss_conflicts
            # removed "cs_search_win_bool[valid_ind] = True"
            cs_search_win_bool[valid_ind] = not (
                (valid_ind == 0) or (valid_ind == cs_search_win_bool.size)
            )
...
def resolve_cs_cs_slow_conflicts(_workingDataBase):
    ...
    _cs_index = _workingDataBase["cs_index"]
    _cs_index_int = np.where(_workingDataBase["cs_index"])[0]
    _workingDataBase["cs_index_slow"] = np.zeros((_cs_index.size), dtype=bool)
    _cs_index_slow = _workingDataBase["cs_index_slow"]
    for counter_cs in range(_cs_index_int.size):
        _cs_index_local = _cs_index_int[counter_cs]
        # if there is not enough data window after the potential CS, then skip it
        if _cs_index_local > (_cs_index.size - window_len):
            _cs_index[_cs_index_local] = False
            continue
        search_win_inds = np.arange(_cs_index_local, _cs_index_local + window_len, 1)
        cs_search_win_data = _data_cs[search_win_inds]
        # find the dominant min/max as the index of the spike
        if _peakType == "max":
...
def align_cs_wrt_ss_index(_workingDataBase):
    ...
    _cs_index_slow = _workingDataBase["cs_index_slow"]
    _cs_index_slow_int = np.where(_workingDataBase["cs_index_slow"])[0]
    _workingDataBase["cs_index"] = np.zeros((_cs_index_slow.size), dtype=bool)
    _cs_index = _workingDataBase["cs_index"]
    _ss_index = _workingDataBase["ss_index"]
    for counter_cs in range(_cs_index_slow_int.size):
...
def align_cs_wrt_ss_temp(_workingDataBase):
    ...
    window_len_before = int(
        (win_look_before + win_ss_template_before) * _workingDataBase["sample_rate"][0]
    )
    window_len_after = int(
        (win_look_after + win_ss_template_after) * _workingDataBase["sample_rate"][0]
    )
    window_len_ss_temp = int(win_ss_template_after * _workingDataBase["sample_rate"][0])
    _cs_index_slow = _workingDataBase["cs_index_slow"]
    _cs_index_slow_int = np.where(_workingDataBase["cs_index_slow"])[0]
    _workingDataBase["cs_index"] = np.zeros((_cs_index_slow.size), dtype=bool)
    _cs_index = _workingDataBase["cs_index"]
...
def align_cs_wrt_cs_temp(_workingDataBase):
    ...
    window_len_before = int(
        (win_look_before + win_cs_template_before) * _workingDataBase["sample_rate"][0]
    )
    window_len_after = int(
        (win_look_after + win_cs_template_after) * _workingDataBase["sample_rate"][0]
    )
    window_len_cs_temp = int(win_cs_template_after * _workingDataBase["sample_rate"][0])
    _cs_index_slow = _workingDataBase["cs_index_slow"]
    _cs_index_slow_int = np.where(_workingDataBase["cs_index_slow"])[0]
    _workingDataBase["cs_index"] = np.zeros((_cs_index_slow.size), dtype=bool)
    _cs_index = _workingDataBase["cs_index"]
...
def filter_data(_workingDataBase):
    _workingDataBase["ch_data_ss"] = lib.bandpass_filter(
    ...
    if _workingDataBase["isLfpSideloaded"][0]:
        _workingDataBase["ch_data_cs"] = lib.bandpass_filter(
...
def detect_ss_index(_workingDataBase):
    _workingDataBase["ss_index"] = lib.find_peaks(
...
def detect_cs_index_slow(_workingDataBase):
    _workingDataBase["cs_index_slow"] = lib.find_peaks(
...
def extract_ss_peak(_workingDataBase):
    _workingDataBase["ss_peak"] = _workingDataBase["ch_data_ss"][
...
def extract_cs_peak(_workingDataBase):
    _workingDataBase["cs_peak"] = _workingDataBase["ch_data_cs"][
...
def extract_ss_waveform(_workingDataBase):
    if _workingDataBase["ss_index"].sum() > 0:
        (
            _workingDataBase["ss_wave"],
            _workingDataBase["ss_wave_span"],
...
def extract_cs_waveform(_workingDataBase):
    if _workingDataBase["cs_index"].sum() > 0:
        (
            _workingDataBase["cs_wave"],
            _workingDataBase["cs_wave_span"],
        ) = lib.extract_waveform(
            _workingDataBase["ch_data_ss"],
...
def extract_ss_ifr(_workingDataBase):
    if _workingDataBase["ss_index"].sum() > 1:
        ...
        _workingDataBase["ss_ifr"] = lib.instant_firing_rate_from_index(
            _workingDataBase["ss_index"], sample_rate=_workingDataBase["sample_rate"][0]
        )
        _workingDataBase["ss_ifr"] = np.append(
            _workingDataBase["ss_ifr"], _workingDataBase["ss_ifr_mean"]
        )
        _workingDataBase["ss_ifr_bins"] = np.linspace(
            _workingDataBase["GLOBAL_IFR_PLOT_SS_MIN"][0],
            _workingDataBase["GLOBAL_IFR_PLOT_SS_MAX"][0],
            _workingDataBase["GLOBAL_IFR_PLOT_SS_BINNUM"][0],
            endpoint=True,
            dtype=np.float32,
        )
        _workingDataBase["ss_ifr_hist"], _ = np.histogram(
            _workingDataBase["ss_ifr"], bins=_workingDataBase["ss_ifr_bins"]
        )
    else:
        ...
        _workingDataBase["ss_ifr_mean"][0] = 0.0
    return 0
...
def extract_cs_ifr(_workingDataBase):
    if _workingDataBase["cs_index"].sum() > 1:
        ...
        _workingDataBase["cs_ifr"] = lib.instant_firing_rate_from_index(
            _workingDataBase["cs_index"], sample_rate=_workingDataBase["sample_rate"][0]
        )
        _workingDataBase["cs_ifr"] = np.append(
            _workingDataBase["cs_ifr"], _workingDataBase["cs_ifr_mean"]
        )
        _workingDataBase["cs_ifr_bins"] = np.linspace(
            _workingDataBase["GLOBAL_IFR_PLOT_CS_MIN"][0],
            _workingDataBase["GLOBAL_IFR_PLOT_CS_MAX"][0],
            _workingDataBase["GLOBAL_IFR_PLOT_CS_BINNUM"][0],
            endpoint=True,
            dtype=np.float32,
        )
        _workingDataBase["cs_ifr_hist"], _ = np.histogram(
            _workingDataBase["cs_ifr"], bins=_workingDataBase["cs_ifr_bins"]
        )
    else:
        ...
        _workingDataBase["cs_ifr_mean"][0] = 0.0
    return 0
...
def extract_ss_xprob(_workingDataBase):
    if _workingDataBase["ss_index"].sum() > 1:
        (
            _workingDataBase["ss_xprob"],
            _workingDataBase["ss_xprob_span"],
...
def extract_cs_xprob(_workingDataBase):
    if _workingDataBase["cs_index"].sum() > 1:
        (
            _workingDataBase["cs_xprob"],
            _workingDataBase["cs_xprob_span"],
        ) = lib.cross_probability(
            _workingDataBase["cs_index"],
            _workingDataBase["ss_index"],
            sample_rate=_workingDataBase["sample_rate"][0],
...
def extract_ss_pca(_workingDataBase):
    ...
    if _workingDataBase["ss_pca_bound_min"][0] > 1:
        _workingDataBase["ss_pca_bound_min"][0] = _workingDataBase[
            "GLOBAL_WAVE_TEMPLATE_SS_BEFORE"
        ][0]
    if _workingDataBase["ss_pca_bound_max"][0] > 1:
        _workingDataBase["ss_pca_bound_max"][0] = _workingDataBase[
            "GLOBAL_WAVE_TEMPLATE_SS_AFTER"
        ][0]

    _minPca = int(
        (
            _workingDataBase["ss_pca_bound_min"][0]
    ...
    _maxPca = int(
        (
            _workingDataBase["ss_pca_bound_max"][0]
            + _workingDataBase["GLOBAL_WAVE_PLOT_SS_BEFORE"][0]
        )
        * _workingDataBase["sample_rate"][0]
    )
    ...
    if _workingDataBase["ss_index"].sum() > 1:
        ...
        if _workingDataBase["umap_enable"][0]:
            ...
            if _workingDataBase["ss_index"].sum() > 15:
                ss_embedding_ = lib.umap(
                    _workingDataBase["ss_wave"][:, _minPca : (_maxPca + 1)]
                )
                _workingDataBase["ss_umap1"] = deepcopy(ss_embedding_[:, 0])
                _workingDataBase["ss_umap2"] = deepcopy(ss_embedding_[:, 1])
            else:
                _workingDataBase["ss_umap1"] = np.random.rand(
                    _workingDataBase["ss_index"].sum()
                )
                _workingDataBase["ss_umap2"] = np.random.rand(
                    _workingDataBase["ss_index"].sum()
                )
        else:
            _workingDataBase["ss_umap1"] = np.zeros(
                (ss_pca_mat_.shape[1]), dtype=np.float32
            )
            _workingDataBase["ss_umap2"] = np.zeros(
                (ss_pca_mat_.shape[1]), dtype=np.float32
            )
    else:
...
def extract_cs_pca(_workingDataBase):
    ...
    if _workingDataBase["cs_pca_bound_min"][0] > 1:
        _workingDataBase["cs_pca_bound_min"][0] = _workingDataBase[
            "GLOBAL_WAVE_TEMPLATE_CS_BEFORE"
        ][0]
    if _workingDataBase["cs_pca_bound_max"][0] > 1:
        _workingDataBase["cs_pca_bound_max"][0] = _workingDataBase[
            "GLOBAL_WAVE_TEMPLATE_CS_AFTER"
        ][0]

    _minPca = int(
        (
            _workingDataBase["cs_pca_bound_min"][0]
    ...
    _maxPca = int(
        (
            _workingDataBase["cs_pca_bound_max"][0]
            + _workingDataBase["GLOBAL_WAVE_PLOT_CS_BEFORE"][0]
        )
        * _workingDataBase["sample_rate"][0]
    )
    ...
    if _workingDataBase["cs_index"].sum() > 1:
        ...
        if _workingDataBase["umap_enable"][0]:
            ...
            if _workingDataBase["cs_index"].sum() > 15:
                cs_embedding_ = lib.umap(
                    _workingDataBase["cs_wave"][:, _minPca : (_maxPca + 1)]
                )
                _workingDataBase["cs_umap1"] = deepcopy(cs_embedding_[:, 0])
                _workingDataBase["cs_umap2"] = deepcopy(cs_embedding_[:, 1])
            else:
                _workingDataBase["cs_umap1"] = np.random.rand(
                    _workingDataBase["cs_index"].sum()
                )
                _workingDataBase["cs_umap2"] = np.random.rand(
                    _workingDataBase["cs_index"].sum()
                )
        else:
            _workingDataBase["cs_umap1"] = np.zeros(
                (cs_pca_mat_.shape[1]), dtype=np.float32
            )
            _workingDataBase["cs_umap2"] = np.zeros(
                (cs_pca_mat_.shape[1]), dtype=np.float32
            )
    else:
...
def extract_ss_template(_workingDataBase):
    if (_workingDataBase["ss_index"].sum() > 0) and (
        _workingDataBase["ssLearnTemp_mode"][0]
    ):
        _ind_begin = int(
            (
                _workingDataBase["GLOBAL_WAVE_PLOT_SS_BEFORE"][0]
        ...
        _workingDataBase["ss_wave_template"] = np.mean(
            _workingDataBase["ss_wave"][:, _window], axis=0
        )
        _workingDataBase["ss_wave_span_template"] = np.mean(
            _workingDataBase["ss_wave_span"][:, _window], axis=0
        )
    else:
...
def extract_cs_template(_workingDataBase):
    if (_workingDataBase["cs_index"].sum() > 0) and (
        _workingDataBase["csLearnTemp_mode"][0]
    ):
        _ind_begin = int(
            (
                _workingDataBase["GLOBAL_WAVE_PLOT_CS_BEFORE"][0]
        ...
        _window = np.arange(_ind_begin, _ind_end, 1)
        _workingDataBase["cs_wave_template"] = np.mean(
            _workingDataBase["cs_wave"][:, _window], axis=0
        )
        _workingDataBase["cs_wave_span_template"] = np.mean(
            _workingDataBase["cs_wave_span"][:, _window], axis=0
        )
    else:
...
def extract_ss_similarity(_workingDataBase):
    ...
    _min_range_len = np.min(
        [(_ind_end_ss_cs - _ind_begin_ss_cs), (_ind_end_cs_cs - _ind_begin_cs_cs)]
    )
    if _workingDataBase["cs_wave_template"].size > 1:
        _min_range_len = np.min(
            [_min_range_len, _workingDataBase["cs_wave_template"].size]
        )
    ...
    if _workingDataBase["ss_index"].sum() > 1:
        ...
        if _workingDataBase["ss_wave_template"].size > 1:
            ...
            _workingDataBase["ss_similarity_to_ss"] = np.array(
                [
                    np.corrcoef(ss_wave[counter, :], ss_wave_template)[0, 1]
                    for counter in range(ss_wave.shape[0])
                ],
                dtype=np.float32,
            )
        else:
            ss_wave_template = np.mean(
                _workingDataBase["ss_wave"][:, _window_ss_ss], axis=0
            )
            _workingDataBase["ss_similarity_to_ss"] = np.array(
                [
                    np.corrcoef(ss_wave[counter, :], ss_wave_template)[0, 1]
        ...
        if _workingDataBase["cs_wave_template"].size > 1:
            ...
            _workingDataBase["ss_similarity_to_cs"] = np.array(
                [
                    np.corrcoef(ss_wave[counter, :], cs_wave_template)[0, 1]
                    for counter in range(ss_wave.shape[0])
                ],
                dtype=np.float32,
            )
        elif _workingDataBase["cs_index"].sum() > 1:
            cs_wave_template = np.mean(
                _workingDataBase["cs_wave"][:, _window_cs_cs], axis=0
            )
            _workingDataBase["ss_similarity_to_cs"] = np.array(
                [
                    np.corrcoef(ss_wave[counter, :], cs_wave_template)[0, 1]
            ...
            _workingDataBase["ss_similarity_to_cs"] = np.zeros(
                (_workingDataBase["ss_index"].sum()), dtype=np.float32
            )
    else:
...
def extract_cs_similarity(_workingDataBase):
    ...
    _min_range_len = np.min(
        [(_ind_end_cs_ss - _ind_begin_cs_ss), (_ind_end_ss_ss - _ind_begin_ss_ss)]
    )
    if _workingDataBase["ss_wave_template"].size > 1:
        _min_range_len = np.min(
            [_min_range_len, _workingDataBase["ss_wave_template"].size]
        )
    ...
    if _workingDataBase["cs_index"].sum() > 1:
        ...
        if _workingDataBase["cs_wave_template"].size > 1:
            ...
            _workingDataBase["cs_similarity_to_cs"] = np.array(
                [
                    np.corrcoef(cs_wave[counter, :], cs_wave_template)[0, 1]
                    for counter in range(cs_wave.shape[0])
                ],
                dtype=np.float32,
            )
        else:
        ...
        if _workingDataBase["ss_wave_template"].size > 1:
            ...
            _workingDataBase["cs_similarity_to_ss"] = np.array(
                [
                    np.corrcoef(cs_wave[counter, :], ss_wave_template)[0, 1]
                    for counter in range(cs_wave.shape[0])
                ],
                dtype=np.float32,
            )
            ...
            _workingDataBase["cs_similarity_to_ss"] = np.zeros(
                (_workingDataBase["cs_index"].sum()), dtype=np.float32
            )
    else:
...
def extract_ss_time(_workingDataBase):
    if _workingDataBase["ss_index"].sum() > 1:
        scale_factor = float(_workingDataBase["sample_rate"][0]) / 1000.0
        _workingDataBase["ss_time"] = _workingDataBase["ch_time"][
            _workingDataBase["ss_index"]
        ]
        ...
        if _workingDataBase["cs_index"].sum() > 1:
            ...
            _workingDataBase["ss_time_to_prev_cs"] = np.zeros(
                (_workingDataBase["ss_index"].sum()), np.float32
            )
            _workingDataBase["ss_time_to_next_cs"] = np.zeros(
                (_workingDataBase["ss_index"].sum()), np.float32
            )
    else:
...
def extract_cs_time(_workingDataBase):
    if _workingDataBase["cs_index"].sum() > 1:
        scale_factor = float(_workingDataBase["sample_rate"][0]) / 1000.0
        _workingDataBase["cs_time"] = _workingDataBase["ch_time"][
            _workingDataBase["cs_index"]
        ]
        ...
        if _workingDataBase["ss_index"].sum() > 1:
            ...
            _workingDataBase["cs_time_to_prev_ss"] = np.zeros(
                (_workingDataBase["cs_index"].sum()), np.float32
            )
            _workingDataBase["cs_time_to_next_ss"] = np.zeros(
                (_workingDataBase["cs_index"].sum()), np.float32
            )
    else:
...
def reset_ss_ROI(_workingDataBase, forced_reset=False):
    is_reset_necessary = not (
        _workingDataBase["ss_index"].sum() == _workingDataBase["ss_index_selected"].size
    )
    if is_reset_necessary or forced_reset:
        ...
        if _workingDataBase["ss_index"].sum() > 1:
            _workingDataBase["ss_index_selected"] = np.zeros(
                (_workingDataBase["ss_index"].sum()), dtype=bool
            )
        else:
            _workingDataBase["ss_index_selected"] = np.zeros((0), dtype=bool)
    return 0


def reset_cs_ROI(_workingDataBase, forced_reset=False):
    is_reset_necessary = not (
        _workingDataBase["cs_index"].sum() == _workingDataBase["cs_index_selected"].size
    )
    if is_reset_necessary or forced_reset:
        ...
        if _workingDataBase["cs_index"].sum() > 1:
            _workingDataBase["cs_index_selected"] = np.zeros(
                (_workingDataBase["cs_index"].sum()), dtype=bool
            )
        else:
            _workingDataBase["cs_index_selected"] = np.zeros((0), dtype=bool)
    return 0
...
def extract_ss_scatter(_workingDataBase):
    if _workingDataBase["ss_index"].sum() > 1:
        ...
        ss_scatter_mat_ = np.hstack(
            (ss_scatter_mat_, _workingDataBase["ss_pca2"].reshape(-1, 1))
        )
        comboBx_Items.append("pca2")
        ss_scatter_mat_ = np.hstack(
            (ss_scatter_mat_, _workingDataBase["ss_pca3"].reshape(-1, 1))
        )
        comboBx_Items.append("pca3")
        ss_scatter_mat_ = np.hstack(
            (ss_scatter_mat_, _workingDataBase["ss_umap1"].reshape(-1, 1))
        )
        comboBx_Items.append("umap1")
        ss_scatter_mat_ = np.hstack(
            (ss_scatter_mat_, _workingDataBase["ss_umap2"].reshape(-1, 1))
        )
        comboBx_Items.append("umap2")
        ss_scatter_mat_ = np.hstack(
            (ss_scatter_mat_, _workingDataBase["ss_peak"].reshape(-1, 1))
        )
        comboBx_Items.append("peak")
        ss_scatter_mat_ = np.hstack(
            (ss_scatter_mat_, _workingDataBase["ss_similarity_to_ss"].reshape(-1, 1))
        )
        comboBx_Items.append("similarity_ss")
        ss_scatter_mat_ = np.hstack(
            (ss_scatter_mat_, _workingDataBase["ss_similarity_to_cs"].reshape(-1, 1))
        )
        comboBx_Items.append("similarity_cs")
        ss_scatter_mat_ = np.hstack(
            (ss_scatter_mat_, _workingDataBase["ss_ifr"].reshape(-1, 1))
        )
        comboBx_Items.append("ifr")
        ss_scatter_mat_ = np.hstack(
            (ss_scatter_mat_, _workingDataBase["ss_time_to_prev_ss"].reshape(-1, 1))
        )
        comboBx_Items.append("t_prev_ss")
        ss_scatter_mat_ = np.hstack(
            (ss_scatter_mat_, _workingDataBase["ss_time_to_next_ss"].reshape(-1, 1))
        )
        comboBx_Items.append("t_next_ss")
        ss_scatter_mat_ = np.hstack(
            (ss_scatter_mat_, _workingDataBase["ss_time_to_prev_cs"].reshape(-1, 1))
        )
        comboBx_Items.append("t_prev_cs")
        ss_scatter_mat_ = np.hstack(
            (ss_scatter_mat_, _workingDataBase["ss_time_to_next_cs"].reshape(-1, 1))
        )
        comboBx_Items.append("t_next_cs")
        ss_scatter_mat_ = np.hstack(
            (ss_scatter_mat_, _workingDataBase["ss_time"].reshape(-1, 1))
        )
        comboBx_Items.append("time")
        ss_scatter_mat_ = np.hstack(
            (
                ss_scatter_mat_,
                np.random.rand(_workingDataBase["ss_pca1"].size).reshape(-1, 1),
            )
        )
        ...
        if _workingDataBase["ss_pca1_index"][0] < num_D:
            _workingDataBase["ss_scatter1"] = _workingDataBase["ss_scatter_mat"][
                :, _workingDataBase["ss_pca1_index"][0]
            ]
        else:
        ...
        if _workingDataBase["ss_pca2_index"][0] < num_D:
            _workingDataBase["ss_scatter2"] = _workingDataBase["ss_scatter_mat"][
                :, _workingDataBase["ss_pca2_index"][0]
            ]
        else:
...
def extract_cs_scatter(_workingDataBase):
    if _workingDataBase["cs_index"].sum() > 1:
        ...
        comboBx_Items.append("pca1")
        cs_scatter_mat_ = np.hstack(
            (cs_scatter_mat_, _workingDataBase["cs_pca2"].reshape(-1, 1))
        )
        comboBx_Items.append("pca2")
        cs_scatter_mat_ = np.hstack(
            (cs_scatter_mat_, _workingDataBase["cs_pca3"].reshape(-1, 1))
        )
        comboBx_Items.append("pca3")
        cs_scatter_mat_ = np.hstack(
            (cs_scatter_mat_, _workingDataBase["cs_umap1"].reshape(-1, 1))
        )
        comboBx_Items.append("umap1")
        cs_scatter_mat_ = np.hstack(
            (cs_scatter_mat_, _workingDataBase["cs_umap2"].reshape(-1, 1))
        )
        comboBx_Items.append("umap2")
        cs_scatter_mat_ = np.hstack(
            (cs_scatter_mat_, _workingDataBase["cs_peak"].reshape(-1, 1))
        )
        comboBx_Items.append("peak")
        cs_scatter_mat_ = np.hstack(
            (cs_scatter_mat_, _workingDataBase["cs_similarity_to_ss"].reshape(-1, 1))
        )
        comboBx_Items.append("similarity_ss")
        cs_scatter_mat_ = np.hstack(
            (cs_scatter_mat_, _workingDataBase["cs_similarity_to_cs"].reshape(-1, 1))
        )
        comboBx_Items.append("similarity_cs")
        cs_scatter_mat_ = np.hstack(
            (cs_scatter_mat_, _workingDataBase["cs_ifr"].reshape(-1, 1))
        )
        comboBx_Items.append("ifr")
        cs_scatter_mat_ = np.hstack(
            (cs_scatter_mat_, _workingDataBase["cs_time_to_prev_ss"].reshape(-1, 1))
        )
        comboBx_Items.append("t_prev_ss")
        cs_scatter_mat_ = np.hstack(
            (cs_scatter_mat_, _workingDataBase["cs_time_to_next_ss"].reshape(-1, 1))
        )
        comboBx_Items.append("t_next_ss")
        cs_scatter_mat_ = np.hstack(
            (cs_scatter_mat_, _workingDataBase["cs_time_to_prev_cs"].reshape(-1, 1))
        )
        comboBx_Items.append("t_prev_cs")
        cs_scatter_mat_ = np.hstack(
            (cs_scatter_mat_, _workingDataBase["cs_time_to_next_cs"].reshape(-1, 1))
        )
        comboBx_Items.append("t_next_cs")
        cs_scatter_mat_ = np.hstack(
            (cs_scatter_mat_, _workingDataBase["cs_time"].reshape(-1, 1))
        )
        comboBx_Items.append("time")
        cs_scatter_mat_ = np.hstack(
            (
                cs_scatter_mat_,
                np.random.rand(_workingDataBase["cs_pca1"].size).reshape(-1, 1),
            )
        )
        comboBx_Items.append("rand num")
        num_D = len(comboBx_Items)
        ...
        if _workingDataBase["cs_pca1_index"][0] < num_D:
            _workingDataBase["cs_scatter1"] = _workingDataBase["cs_scatter_mat"][
                :, _workingDataBase["cs_pca1_index"][0]
            ]
        else:
        ...
        if _workingDataBase["cs_pca2_index"][0] < num_D:
            _workingDataBase["cs_scatter2"] = _workingDataBase["cs_scatter_mat"][
                :, _workingDataBase["cs_pca2_index"][0]
            ]
        else:

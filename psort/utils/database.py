...
class PsortDataBase:
    ...
    def init_slotsDataBase_hard(
        self, ch_data=None, ch_time=None, sample_rate=30000, index_slot_edges=None
    ):
        if ch_data is None:
        ...
        if index_slot_edges is None:
            ...
            slot_duration = 100.0
            total_slot_num = int(
                np.ceil(float(data_size) / float(sample_rate) / slot_duration)
            )
            index_slot_edges = np.round(
                np.linspace(0, data_size, total_slot_num + 1, endpoint=True)
            ).astype(int)
        ...
        for counter_slot in range(total_slot_num):
            self._grandDataBase.append(deepcopy(dictionaries._singleSlotDataBase))
            self._grandDataBase[counter_slot]["index_start_on_ch_data"][
                0
            ] = index_slot_edges[counter_slot]
            self._grandDataBase[counter_slot]["index_end_on_ch_data"][
                0
        ...
        self._topLevelDataBase["ss_index"] = np.zeros(
            (self._topLevelDataBase["ch_data"].size), dtype=bool
        )
        self._topLevelDataBase["cs_index"] = np.zeros(
            (self._topLevelDataBase["ch_data"].size), dtype=bool
        )
        self._topLevelDataBase["cs_index_slow"] = np.zeros(
            (self._topLevelDataBase["ch_data"].size), dtype=bool
        )
            ...
            for key in dictionaries._singleSlotDataBase.keys():
                if not (key in self._grandDataBase[-2].keys()):
                    for counter_slot in range(len(self._grandDataBase) - 1):
                        self._grandDataBase[counter_slot][key] = deepcopy(
                            dictionaries._singleSlotDataBase[key]
                        )
    ...
    def sideload_lfp(self, file_fullPath, ch_data=None, ch_time=None, sample_rate=None):
        ...
        if _ch_data_lfp_orig.size == _ch_data.size:
            ...
            _ch_data_lfp_orig = lib.resample(
                x_input=_ch_time_lfp_orig, y_input=_ch_data_lfp_orig, x_output=_ch_time
            )
            self._topLevelDataBase["ch_lfp"] = deepcopy(_ch_data_lfp_orig)
        return 0
    ...
    def update_dataBase_based_on_signals(self, guiSignals_workingDataBase):
        ...
        psortDataBase_topLevel["cs_index"][
            index_start_on_ch_data:index_end_on_ch_data
